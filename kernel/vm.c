...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    // vriskw pagetable entry - pte gia tn sigkekrimeni selida
    ...
    if(((*pte) & PTE_W) == 0)
    {
      // meta apo autous tous elegxous
      // na dimiourgithei nea selida
      if((new_pa = kalloc()) == 0)
        return -1;

      // na antigrafei i palia selida stn nea
      ...
      *pte |= PTE_W;
      ...
      // "apodesmeusi" tis proigoumenis read-only selidas
      // an to reference_count ginei 0
      kfree((void*)pa0);

      n = PGSIZE - (dstva - va0);
      if(n > len)
        n = len;
      // antikatastasi pa0 me new_pa
